...
The high-rate inner loop additionally enforces a no-climb descent envelope from 200 m down to the 7 m terminal handoff. Whenever measured vertical velocity is slower than the negative descent reference, commanded net vertical acceleration is capped to a downward value between 0.5 and 3.0 m/s², proportional to the velocity mismatch. This guard applies to both accepted async trajectories and async PD fallback, so neither ownership path can turn a delayed braking correction into sustained ascent.
