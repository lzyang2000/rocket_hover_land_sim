...
The controller moves above the pad and stabilizes position, velocity, attitude, and angular rate. Rather than jumping the reference directly to the pad, guidance uses a 4 m horizontal lead through 18 m altitude, expands it by 0.15 m per additional metre, and caps it at 8 m. The vertical lead remains 2 m. This altitude schedule gives a high vehicle faster lateral capture while retaining the proven low-altitude response. ALIGN is a capture phase rather than a requirement to hover nearly over pad center. Its lateral capture radius remains 2 m through 18 m altitude, then expands by 0.15 m per additional metre and is capped at 8 m. The normal capture gate also requires horizontal speed below 1.0 m/s, staging-altitude error below 2 m, and vertical speed below 1.5 m/s.

Outside that nominal corridor, a high-altitude state may enter DESCEND only if a cubic lateral transfer can satisfy the measured initial position and velocity and reach zero position and velocity at the 7 m terminal handoff. The controller evaluates the acceleration at both ends of that cubic and requires the larger magnitude to remain below 1.5 m/s². This replaces the unsafe time-based transition: a recoverable trajectory can descend while aligning, but a state that cannot meet the terminal corridor remains in ALIGN. Horizontal correction continues through the altitude-scheduled lead, while the much tighter touchdown gates remain unchanged.
