...
The simulator process does not hot-reload Python or MJCF changes. Close every existing simulator window before relaunching. The current window title should contain `v0.9.13`.
...
The state machine supplies moving position and velocity references to the 6-DOF MPC. At 18 m altitude and below, ALIGN uses a four-metre horizontal and two-metre vertical lead instead of jumping directly to the pad. Above 18 m, the horizontal lead grows by 0.15 m per additional metre, capped at 8 m, so high-altitude lateral capture uses more of the available authority without changing the low-altitude controller. The descent-capture radius separately stays at 2 m through 18 m, then grows by 0.15 m per additional metre and is capped at 8 m. Descent normally starts inside that corridor once horizontal speed is below 1.0 m/s and staging-altitude error and vertical speed are below 2 m and 1.5 m/s. Above 18 m, ALIGN has a five-second maximum so it cannot remain stuck indefinitely. If that timeout is reached with substantial lateral error or speed, DESCEND begins at a proportionally reduced vertical rate while lateral correction continues through the altitude-scheduled bounded lead. It then uses an aggressive approach with terminal braking:
